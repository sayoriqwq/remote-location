...
The developer-side participant that applies a selected location to an active Xcode-managed test session on a paired device.
...
The single physical iPhone currently selected for the developer test session and eligible to receive simulation requests.
_Avoid_: Device Fleet, Concurrent Target

**Simulation Capability（模拟能力）**:
The consumer-agnostic ability to request, apply, stop, and report a Simulated Location through an active developer test session. It does not include validating how any particular app consumes that location.
